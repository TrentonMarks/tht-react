...
/**
 * @file
 * Contains Drupal\overseer\Form\OverseerBasicSettingsForm.
 */
...
class OverseerBasicSettingsForm extends ConfigFormBase {
  /**
   * {@inheritdoc}
   */
   ...
   */
   public function buildForm(array $form, FormStateInterface $form_state) {
     ...
     /* Query all Basic pages */
     $db = \Drupal::database(); // Drupal database
     $query_node_data = $db->query("SELECT * FROM {node_field_data} WHERE type = 'basic'"); // query for all Basic pages
     $basic_pages = $query_node_data->fetchAll(); // run query

     /* Form Description */
     $form['description'] = array(
       '#type' => 'item',
       '#title' => $this->t('Overseer settings'),
       '#description' => $this->t('Override the Hero image for the Basic page. Clear caches after saving for changes to take effect.'),
     );

     /* Loop: Create a set of fields for each Basic page */
     foreach ($basic_pages as $page) {
       $nid = $page->nid; // node id

       /* Get URL to current Basic page */
       $current_node_path = '/node/'.$nid; // Basic page's path by its nid
       $query_path_data = $db->query("SELECT * FROM {path_alias} WHERE path = '$current_node_path'"); // query for Basic page's path alias
       $path = $query_path_data->fetchAll(); // run query

       /* Form Details */
       $form['basic_page-'.$nid] = array(
         '#id' => 'basic_page-'.$nid,
         '#type' => 'details',
         '#title' => $this->t($page->title),
         '#description' => $this->t('<a href="'.$path[0]->alias.'">View page</a>'), // link to Basic page
         '#open' => TRUE,
       );

       /* File upload field for Hero image */
       $form['basic_page-'.$nid][$nid] = array(
         '#type' => 'managed_file',
         '#name' => 'Hero',
       ...
       /* Alt text field */
       $form['basic_page-'.$nid]['alt_text-'.$nid] = array(
         '#type' => 'textfield',
       ...
       /* Title */
       $form['basic_page-'.$nid]['title-'.$nid] = array(
         '#type' => 'textfield',
         '#title' => $this->t('Title'),
       ...
       /* Body */
       $form['basic_page-'.$nid]['body-'.$nid] = array(
         '#type' => 'textarea',
         '#title' => $this->t('Body'),
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    /* Query all Basic pages */
    $db = \Drupal::database();
    $query = $db->query("SELECT * FROM {node_field_data} WHERE type = 'basic'");
    $result = $query->fetchAll();

    /* Loop: Through each existing Basic page */
