...
/**
 * @file
 * Contains Drupal\overseer\Form\OverseerPersonSettingsForm.
 */
...
class OverseerPersonSettingsForm extends ConfigFormBase {
  /**
   * {@inheritdoc}
   */
   ...
   */
   public function buildForm(array $form, FormStateInterface $form_state) {
     ...
     /* Query all Person pages */
     $db = \Drupal::database(); // Drupal database
     $query_node_data = $db->query("SELECT * FROM {node_field_data} WHERE type = 'person'"); // query for all Person pages
     $person_pages = $query_node_data->fetchAll(); // run query

     /* Form Description */
     $form['description'] = array(
       '#type' => 'item',
       '#title' => $this->t('Overseer settings'),
       '#description' => $this->t('Override the Hero image for the Person page. Clear caches after saving for changes to take effect.'),
     );

     /* Loop: Create a set of fields for each Person page */
     foreach ($person_pages as $page) {
       $nid = $page->nid; // node id

       /* Get URL to current Person page */
       $current_node_path = '/node/'.$nid; // Person page's path by its nid
       $query_path_data = $db->query("SELECT * FROM {path_alias} WHERE path = '$current_node_path'"); // query for Person page's path alias
       $path = $query_path_data->fetchAll(); // run query

       /* Form Details */
       $form['person_page-'.$nid] = array(
         '#id' => 'person_page-'.$nid,
         '#type' => 'details',
         '#title' => $this->t($page->title),
         '#description' => $this->t('<a href="'.$path[0]->alias.'">View page</a>'), // link to Person page
         '#open' => TRUE,
       );

       /* File upload field for Hero image */
       $form['person_page-'.$nid][$nid] = array(
         '#type' => 'managed_file',
         '#name' => 'Hero',
       ...
       /* Alt text field */
       $form['person_page-'.$nid]['alt_text-'.$nid] = array(
         '#type' => 'textfield',
       ...
       /* Title */
       $form['person_page-'.$nid]['title-'.$nid] = array(
         '#type' => 'textfield',
         '#title' => $this->t('Title'),
       ...
       /* Body */
       $form['person_page-'.$nid]['body-'.$nid] = array(
         '#type' => 'textarea',
         '#title' => $this->t('Body'),
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    /* Query all Person pages */
    $db = \Drupal::database();
    $query = $db->query("SELECT * FROM {node_field_data} WHERE type = 'person'");
    $result = $query->fetchAll();

    /* Loop: Through each existing Person page */
