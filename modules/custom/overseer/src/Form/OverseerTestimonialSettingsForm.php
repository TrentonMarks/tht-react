...
/**
 * @file
 * Contains Drupal\overseer\Form\OverseerTestimonialSettingsForm.
 */
...
class OverseerTestimonialSettingsForm extends ConfigFormBase {
  /**
   * {@inheritdoc}
   */
   ...
   */
   public function buildForm(array $form, FormStateInterface $form_state) {
     ...
     /* Query all Testimonial pages */
     $db = \Drupal::database(); // Drupal database
     $query_node_data = $db->query("SELECT * FROM {node_field_data} WHERE type = 'testimonial'"); // query for all Testimonial pages
     $testimonial_pages = $query_node_data->fetchAll(); // run query

     /* Form Description */
     $form['description'] = array(
       '#type' => 'item',
       '#title' => $this->t('Overseer settings'),
       '#description' => $this->t('Override the Hero image for the Testimonial page. Clear caches after saving for changes to take effect.'),
     );

     /* Loop: Create a set of fields for each Testimonial page */
     foreach ($testimonial_pages as $page) {
       $nid = $page->nid; // node id

       /* Get URL to current Testimonial page */
       $current_node_path = '/node/'.$nid; // Testimonial page's path by its nid
       $query_path_data = $db->query("SELECT * FROM {path_alias} WHERE path = '$current_node_path'"); // query for Testimonial page's path alias
       $path = $query_path_data->fetchAll(); // run query

       /* Form Details */
       $form['testimonial_page-'.$nid] = array(
         '#id' => 'testimonial_page-'.$nid,
         '#type' => 'details',
         '#title' => $this->t($page->title),
         '#description' => $this->t('<a href="'.$path[0]->alias.'">View page</a>'), // link to Testimonial page
         '#open' => TRUE,
       );

       /* File upload field for Hero image */
       $form['testimonial_page-'.$nid][$nid] = array(
         '#type' => 'managed_file',
         '#name' => 'Hero',
       ...
       /* Alt text field */
       $form['testimonial_page-'.$nid]['alt_text-'.$nid] = array(
         '#type' => 'textfield',
       ...
       /* Title */
       $form['testimonial_page-'.$nid]['title-'.$nid] = array(
         '#type' => 'textfield',
         '#title' => $this->t('Title'),
       ...
       /* Body */
       $form['testimonial_page-'.$nid]['body-'.$nid] = array(
         '#type' => 'textarea',
         '#title' => $this->t('Body'),
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    /* Query all Testimonial pages */
    $db = \Drupal::database();
    $query = $db->query("SELECT * FROM {node_field_data} WHERE type = 'testimonial'");
    $result = $query->fetchAll();

    /* Loop: Through each existing Testimonial page */
